...
export const CHANNELS = ['INAPP', 'SMS', 'TELEGRAM', 'BALE', 'EITAA', 'SOROUSH', 'IGAP'] as const;
export type Channel = typeof CHANNELS[number];

export const CHANNEL_LABEL: Record<string, string> = {
  INAPP: 'پیام درون‌سامانه', SMS: 'پیامک', TELEGRAM: 'تلگرام', BALE: 'بله', EITAA: 'ایتا',
  SOROUSH: 'سروش', IGAP: 'ای‌گپ',
};
...
async function messengerConfig(channel: Channel) {
  ...
  if (channel === 'IGAP') return { token: await getSetting('IGAP_TOKEN'), base: (await getSetting('IGAP_API_BASE')) || 'https://igap.ai/api', style: 'BOT' as const };
