...
## ⚙️ پیکربندی — هیچ مقداری در کد ثابت نیست

نشانی سرویس‌ها، کلیدها و توکن‌ها (BigBlueButton، Moodle، پیامک، درگاه پرداخت، ایرانداک، سجاد و…)
هیچ‌کدام در کد هارد‌کد نشده‌اند. هر مقدار با این ترتیب خوانده می‌شود:

```
۱) پنل مدیر ← ⚙️ پیکربندی سامانه        (جدول system_settings — بدون ری‌استارت اعمال می‌شود)
۲) متغیر محیطی (ENV / فایل .env)        (پیش‌فرض استقرار)
۳) مقدار پیش‌فرض تعریف‌شده در کد        (معمولاً خالی = سرویس غیرفعال)
```

- صفحهٔ **`/admin/settings`** همهٔ کلیدها را گروه‌بندی‌شده نشان می‌دهد و کنار هر مقدار برچسب
  منبعش را می‌زند: «تنظیم‌شده در پنل» / «از ENV» / «پیش‌فرض». دکمهٔ «بازنشانی» مقدار پنل را
  حذف می‌کند تا دوباره ENV ملاک شود.
- مقادیر محرمانه (کلید BBB، توکن ربات‌ها، کلید پذیرنده) **هرگز به مرورگر ارسال نمی‌شوند**؛
  به‌صورت `••••••••` نمایش داده می‌شوند و اگر دست‌نخورده بمانند بازنویسی نمی‌شوند.
- تب‌های «کلاس مجازی»، «پیامک و ربات‌ها» و «درگاه پرداخت» در صفحهٔ قالب‌ها هم روی همین
  ذخیره‌سازی می‌نویسند.
- گروه **زیرساخت** (`DATABASE_URL`، `REDIS_URL`، `S3_*`، `PORT`) فقط از ENV خوانده می‌شود،
  چون پیش از اتصال به دیتابیس لازم است؛ در پنل فقط نمایش داده می‌شود.

نمونهٔ کامل کلیدها با نام متغیر محیطی معادل: [`afagh-next/.env.example`](afagh-next/.env.example)
...
## انتقال دادهٔ سیستم قدیمی

بعد از بالا آمدن سامانه، از پنل مدیر ← **🔄 انتقال داده از سیستم قدیمی** (`/admin/migration`) می‌توانید
داده‌های سرور قدیمی را با فایل **اکسل** منتقل کنید: دادهٔ پایه، تطبیق کدها (رشته/مقطع/ترم/درس)،
فرمول‌های شهریه + مقایسه با دادهٔ مالی قدیمی، و نمرات (با مقایسهٔ پیش از اعمال).

هر فایل ابتدا خام در **ناحیهٔ موقت** ذخیره می‌شود؛ بنابراین سطرهای خطادار را می‌توان بعد از تکمیل
نگاشت کدها **دوباره پردازش کرد** (بدون آپلود مجدد) و هر چیزی که روی سامانه نوشته شده از تب
**«↩ ناحیهٔ موقت و واگرد»** قابل **برگشت** است.
راهنمای کامل و ستون‌های هر فایل: [`docs/MIGRATION.md`](docs/MIGRATION.md)

## پویش خودکار فارغ‌التحصیلی (اختیاری)

ماژول فارغ‌التحصیلی با هر رویداد ثبت نمره خودش اجرا می‌شود؛ برای اطمینان می‌توانید یک
اجرای شبانه هم تعریف کنید. ابتدا در `/admin/settings` ← «فارغ‌التحصیلی و صدور مدارک»
مقدار `GRAD_CRON_SECRET` را تعیین کنید، سپس روی سرور:

```bash
crontab -e
0 3 * * * curl -fsS -X POST -H "x-cron-secret: <همان کلید>" http://127.0.0.1:8080/api/cron/graduation-scan
```
