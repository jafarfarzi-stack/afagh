...
| **۳ — Server Actions** | ✅ **انجام شد**: `src/app/admin/curriculum/actions/` با ۲۵ اکشن گارددار در ۵ فایل (read / courses / versions / rules / lifecycle) + لایهٔ مشترک `shared.ts` — در دور ۱۳ از یک فایل ۱۱۸۰ سطری جدا شد (۲ متدویه: الگوی D3) + `src/lib/curriculum-validator.ts` (هستهٔ واقعی ۶ چک — گیت تأیید بدون استاب) + `tests/curriculum-validator.test.ts` (۱۹ تست) | ~۹۵۰ خط |
