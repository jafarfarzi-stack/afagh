...
## ۴) کارنامهٔ عملکرد استاد — زنده، بدون عدد ثابت

مسیر کد: `afagh-next/src/lib/professor-performance.ts`
صفحه: `/professor/performance`

این صفحه پیش‌تر از `sampleStaffPerformance` (دادهٔ نمونه در
`executive-analytics`) تغذیه می‌شد و اعداد مقایسه‌ای‌اش در JSX سخت‌کد بودند
(«مقام … از ۳۸ استاد»، «میانگین ۸۶٪»، «★۴.۲»، «۱۳.۵ ساعت در برابر ۲۶ ساعت»).
حالا همهٔ اعداد از پایگاه داده می‌آیند:

| شاخص | منبع داده |
|---|---|
| کلاس/دانشجو/کد درس | `course_offerings` + `enrollments` (یک کوئری تجمیعی) |
| جلسات برگزار/برگزارنشده/جبرانی و نرخ برگزاری | `class_sessions` (یک `GROUP BY`) |
| نمرات ثبت‌شده/در انتظار، مهلت، اعتراض باز | `enrollments` + `grade_appeals` |
| MTTR، پایبندی SLA، نقض/ارجاع، صف باز، CSAT، رشد ماهانه | `request_step_logs` (یک `GROUP BY` روی همهٔ اساتید) |
| نمرهٔ ارزشیابی، پاسخ‌دهندگان، روند، رتبه، میانگین همکاران | داشبورد **کش‌شدهٔ** BI (`managementOverview`) |
| میانگین گروه آموزشی | یک کوئری تجمیعی با `avg(...)` روی `offering_professors`/`course_offerings` |

* «میانگین دانشگاه» که قبلاً عدد ثابت بود، حالا **میانگین واقعی همکاران دارای
  اقدام در همین ترم** است و از همان کوئری `GROUP BY` به دست می‌آید (بدون کوئری
  اضافه).
* رتبهٔ ارزشیابی و رتبهٔ سرعت پاسخگویی هر دو از دادهٔ واقعی محاسبه می‌شوند.
* نشان عملکرد و استحقاق پاداش از چهار آستانهٔ تنظیم‌شدنی می‌آیند
  (`PERF_SLA_TARGET`، `PERF_EVAL_TARGET`، `PERF_SESSION_HOLD_TARGET`،
  `PERF_INCENTIVE_PERCENT`) و در صورت عدم استحقاق، **دلیل دقیق** هر شاخص به
  استاد نشان داده می‌شود.
* هزینه: **۱۱ کوئری** برای کل کارنامه (بدون N+1) — پیش از بهینه‌سازی، صدا زدن
  `professorPanel` کنار داشبورد BI باعث ۲۶ کوئری می‌شد؛ روند و پاسخ‌دهندگان از
  داشبورد کش‌شده برداشته شد و فقط میانگین گروه با یک کوئری جدا گرفته می‌شود.
